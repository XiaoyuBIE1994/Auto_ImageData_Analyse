...
def model_run (file, data_color_range = [60, 160, 160, 210, 130, 200], start_ratio = 0.5, \
               vert_ratio_num = 0.5, vert_ratio_gap=0.5, ransac_params = {}, \
               find_corner_params = {}, find_mark_params = {}, find_coor_params = {}, find_peak_params = {},\
               MODE_CHECK = False, SHOW_RANSAC_LINE = False, CHECK_RESULT = False, SHOW_RESULT = False, peakvalue ={}):
    ...
    X, y = ImageProcess.color_filter(img, data_color_range)
    ...
    while loop_ransac:
        try:
            ...
            if SHOW_RANSAC_LINE:
                ...
                plt.show()
                cv2.imwrite("D:\\PFE_Xue\\04-Mise_en_oeuvre_essais\\99-Archive\\Archive\\"+file_name[:-4]+"T.PNG", cv2.cvtColor(img_ransac, cv2.COLOR_RGB2BGR))
    ...
    #         peak_value = OCR.find_peak(img, dic_corners, find_peak_params)
            peak_value =peakvalue[file_name[:-4]]
        except ValueError:
